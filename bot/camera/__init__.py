...
class CameraServo:
    ...
    #                 if len(path) > 3 and (
    #                     path[3] == "roll" or path[3] == "0"):  # mutinus/control/camera/0/#
    #                     if len(path) > 4 and path[4] == "deg":  # mutinus/control/camera/0/deg
    #                         self.set_position_degrees(0, degrees, self.ROLL_MID, self.ROLL_MIN,
    #                                                   self.ROLL_MAX,
    #                                                   self.ROLL_DEG)
    #                     elif len(path) > 4 and path[4] == "percent":
    #                         self.set_position_percent(0, int(msg.payload), self.ROLL_MIN,
    #                                                   self.ROLL_MAX)
    #                     else:  # mutinus/control/camera/roll
    #                         self.set_position(0, int(msg.payload), self.ROLL_MIN, self.ROLL_MAX)
    #                 if len(path) > 3 and (
    #                     path[3] == "pitch" or path[3] == "1"):  # mutinus/control/camera/0/#
    #                     if len(path) > 4 and path[4] == "deg":  # mutinus/control/camera/0/deg
    #                         self.set_position_degrees(1, degrees, self.PITCH_MID, self.PITCH_MIN,
    #                                                   self.PITCH_MAX,
    #                                                   self.PITCH_DEG)
    #                     elif len(path) > 4 and path[4] == "percent":
    #                         self.set_position_percent(1, int(msg.payload), self.PITCH_MIN,
    #                                                   self.PITCH_MAX)
    ...
    def set_position(self, servo, position, position_min, position_max):
        ...
        #
        # self.client.publish(self.serviceName + "/state/camera/" + str(servo) + "/raw", position, 0,
        #                     True)
        #
        # percent = 100 - int(
        #     float(position - position_min) * 100.0 / float(position_max - position_min))
        # self.client.publish(self.serviceName + "/state/camera/" + str(servo) + "/percent", percent,
        #                     1, True)
        #
    ...
    def set_position_percent(self, servo, percent, position_min, position_max):
        ...
        position = position_min + int(
            (100.0 - float(percent)) * float(position_max - position_min) / 100.0)
        # self.logger.debug(str(percent) + "% -> " + str(position))
        self.set_position(servo, position, position_min, position_max)

    def set_position_degrees(self, servo, degrees, mid, position_min, position_max,
                             points_per_degree):
